...
import { useAuthStore, useHydrationStore, useSleepStore, useHabitsStore, useNutritionStore, useInsightStore } from '../../store';
import { hydrationService, sleepService, habitsService, nutritionService, insightsService } from '../../services/api';
    ...
    >
      ...
      >
        ...
        <Text style={styles.cardStat}>
          {(hydration.todayTotal / 1000).toFixed(1)}L of {(hydration.goal / 1000).toFixed(1)}L goal
        </Text>
      </TouchableOpacity>
        ...
        <View style={styles.sleepStats}>
          <View style={styles.sleepStat}>
            <Text style={styles.sleepStatValue}>
              {sleep.lastNight ? `${sleep.lastNight.duration_hrs.toFixed(1)}h` : '—'}
            </Text>
          ...
          <View style={styles.sleepDivider} />
          <View style={styles.sleepStat}>
            <Text style={styles.sleepStatValue}>{profile?.sleep_goal_hrs || 8}h</Text>
      ...
      <View style={styles.row}>
        ...
        >
          ...
          <Text style={styles.bigStat}>{habits.completedCount()}/{habits.totalCount()}</Text>
          <View style={styles.progressBarBg}>
            <View style={[styles.progressBarFill, {
        ...
        >
          ...
          <Text style={styles.macroLine}>
            P {Math.round(nutrition.todayTotals.protein_g)}g · C {Math.round(nutrition.todayTotals.carbs_g)}g · F {Math.round(nutrition.todayTotals.fat_g)}g
          </Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  name: { color: Colors.text.primary, fontSize: Typography.size.xxl, fontWeight: Typography.weight.bold },

  voiceBtn: { borderRadius: Radius.full, overflow: 'hidden', ...Shadows.glow },
  voiceBtnGrad: { width: 52, height: 52, alignItems: 'center', justifyContent: 'center' },
  ...
  cardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: Spacing.base },
  cardIcon:  { fontSize: 20, marginRight: Spacing.sm },
  cardTitle: { flex: 1, color: Colors.text.secondary, fontSize: Typography.size.sm, fontWeight: Typography.weight.medium, textTransform: 'uppercase', letterSpacing: 0.8 },
  cardBadge: { fontSize: Typography.size.sm, fontWeight: Typography.weight.bold },

  progressBarBg: { height: 4, backgroundColor: Colors.bg.border, borderRadius: 2, marginBottom: Spacing.sm, overflow: 'hidden' },
  ...
  sleepStatValue: { color: Colors.text.primary, fontSize: Typography.size.xl, fontWeight: Typography.weight.bold },
  sleepStatLabel: { color: Colors.text.tertiary, fontSize: Typography.size.xs, marginTop: 2 },
  sleepDivider: { width: 1, height: 32, backgroundColor: Colors.bg.border },

  row: { flexDirection: 'row', gap: Spacing.sm },
  halfCard: { flex: 1, marginBottom: 0 },

  bigStat: { color: Colors.text.primary, fontSize: Typography.size.xxl, fontWeight: Typography.weight.bold, marginVertical: Spacing.xs },
  macroLine: { color: Colors.text.tertiary, fontSize: Typography.size.xs, marginTop: Spacing.xs },
});
